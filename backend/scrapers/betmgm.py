...
async def scrape_betmgm(sport: str) -> list:
    """Scrape moneyline odds from BetMGM for *sport*.

    Returns a list of dicts: ``[{"event", "outcome", "odds", "book"}, ...]``
    """
    url = BETMGM_URLS.get(sport)
    if not url:
        return []
    ...
    try:
        async with async_playwright() as p:
            ...
            # BetMGM event containers – selectors may change over time
            event_rows = soup.select(
                "ms-event, "
                "[class*='event-card'], "
                "[class*='participant-pair']"
            )
        ...
        print(f"[BetMGM] Scrape error ({sport}): {exc}")
